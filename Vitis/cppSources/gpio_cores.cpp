...
void GpoCore::write_led0(uint32_t led_blink_speed_ms) {
   wr_data = led_blink_speed_ms;
   io_write(base_addr, LED0_DATA_REG, wr_data);
}

void GpoCore::write_led1(uint32_t led_blink_speed_ms) {
   wr_data = led_blink_speed_ms;
   io_write(base_addr, LED1_DATA_REG, wr_data);
}

void GpoCore::write_led2(uint32_t led_blink_speed_ms) {
   wr_data = led_blink_speed_ms;
   io_write(base_addr, LED2_DATA_REG, wr_data);
}

void GpoCore::write_led3(uint32_t led_blink_speed_ms) {
   wr_data = led_blink_speed_ms;
   io_write(base_addr, LED3_DATA_REG, wr_data);
}

void GpoCore::write_led_test(uint32_t led_blink_speed_ms) {
   wr_data = led_blink_speed_ms;
   io_write(base_addr, DATA_REG, wr_data);
}
